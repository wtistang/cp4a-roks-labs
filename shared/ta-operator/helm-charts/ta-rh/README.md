...
```bash
helm install --name my-release --set authentication.ocp.edgeIp=<edgeIP> --set authentication.ocp.secretName=<my-secret> stable/ibm-transadv-dev --tls
```
...
> **Note**: Those parameters are required for install `authentication.ocp.edgeIp`, `authentication.ocp.secretName`
...
| authentication.ocp.edgeIp                           | edge node IP or full domain name                             | ""                                                      | 
| authentication.ocp.endpointPort                     | edge node login port                                         | 8443 _(1)_                                                    |
| authentication.ocp.secretName                       | The name of the secret in the Configuration in the same namespace of this release to be installed to| ""               |
...
_(1)_ The edge node login port can be varied when ocp is configured and installed. In ICP 3.1.2 or later, the port information can be found in the left navigation in ICP console UI, Configuration > ConfigMaps; 
